"""
v17c: Soft Pseudo-Labels + Multi-Strategy Blending — HONEST
=============================================================
Key difference from v17b:
  - Uses CONTINUOUS ensemble predictions as pseudo-labels (not hard assignments)
  - Avoids error propagation from discretization
  - Blends predictions from multiple rounds
  - Confidence-weighted pseudo-labels (high-confidence predictions weighted more)
"""
import re, os, time
import numpy as np
import pandas as pd
...
# ════════════════════════════════════════════════════════════════
print("="*70)
print("v17c: Soft Pseudo-Labels + Multi-Strategy — HONEST")
print("="*70)
...
# ════════════════════════════════════════════════════════════════
#  ROUND 0: DIRECT PREDICTION (249 → 91)
# ════════════════════════════════════════════════════════════════
print(f"\n{'='*70}")
print("Round 0: Direct prediction")
print("="*70)
...
for tp in [0.05, 0.10, 0.15]:
    nt_ = max(1, int(M0.shape[1] * tp))
    if M0.shape[1] > 2*nt_:
        r0_trim = np.mean(M0_sorted[:, nt_:-nt_], axis=1)
        for pw in [1.0, 1.1, 1.25, 1.5]:
            a = hungarian_assign(r0_trim, test_seasons, avail_seeds, pw)
            ex, sse, rmse = evaluate(a, test_gt)
            print(f"  R0 trim{int(tp*100):02d}+p{pw}: {ex}/91 RMSE={rmse:.4f}")

# SOFT pseudo-labels = continuous ensemble prediction (NOT assigned)
soft_pseudo = r0_mean.copy()  # continuous predictions
print(f"\n  Soft pseudo-labels: mean={soft_pseudo.mean():.1f}, std={soft_pseudo.std():.1f}")


# ════════════════════════════════════════════════════════════════
#  ROUNDS 1-3: SOFT PSEUDO-LABEL LOO
# ════════════════════════════════════════════════════════════════
all_round_preds = [r0_mean.copy()]  # accumulate predictions from each round
best_exact = 0; best_rmse = 999; best_assigned = None

for rnd in range(1, 4):
    print(f"\n{'='*70}")
    print(f"Round {rnd}: Soft Pseudo-Label LOO (340 pooled, CONTINUOUS targets)")
    print("="*70)
    
    # Pool: 249 train (true labels) + 91 test (SOFT continuous pseudo-labels)
    P_X = np.vstack([X_tr_imp, X_te_tourn_imp])
    P_y = np.concatenate([y_train, soft_pseudo])
    P_seas = np.concatenate([train_seasons, test_seasons])
    
    # Model confidence: how much variance across models for each test team?
    if rnd == 1:
        model_std = np.std(M0, axis=1)
    else:
        # Use std from previous round
        model_std = np.std(np.column_stack(all_round_preds[-3:]), axis=1) if len(all_round_preds) >= 3 else np.ones(n_te)
    
    # Sample weight: true labels get weight 1.0, pseudo labels get weight based on confidence
    # High variance = low confidence = lower weight
    pseudo_weight = 1.0 / (1.0 + model_std / 10.0)
    sample_weights = np.concatenate([np.ones(n_tr), pseudo_weight])
    ...
    for ti in range(n_te):
        ...
        for fs_name, fs_idx in FS.items():
            ...
            wf = w_fold  # sample weights
            
            # XGB with sample weights
            for d in [3,5,7]:
                for lr in [0.03,0.1]:
                    m = xgb.XGBRegressor(n_estimators=150,max_depth=d,learning_rate=lr,
                        reg_lambda=1.0,reg_alpha=0.1,colsample_bytree=0.8,subsample=0.8,
                        random_state=42,verbosity=0,tree_method='hist')
                    m.fit(Xf, y_fold, sample_weight=wf)
                    loo_preds[f'xgb_d{d}_lr{lr}_{fs_name}'][ti] = m.predict(Xtf)[0]
            # LGB with sample weights
            for d in [3,5]:
                for mc in [5,10]:
                    m = lgb.LGBMRegressor(n_estimators=150,max_depth=d,learning_rate=0.05,
                        min_child_samples=mc,reg_lambda=1.0,random_state=42,verbose=-1)
                    m.fit(Xf, y_fold, sample_weight=wf)
                    loo_preds[f'lgb_d{d}_mc{mc}_{fs_name}'][ti] = m.predict(Xtf)[0]
            # HGBR (no sample_weight param in fit, but has it)
            for d in [3,6]:
                m = HistGradientBoostingRegressor(max_depth=d,learning_rate=0.05,max_iter=150,random_state=42)
                m.fit(Xf, y_fold, sample_weight=wf)
                loo_preds[f'hgbr_d{d}_{fs_name}'][ti] = m.predict(Xtf)[0]
            # Ridge (no sample weight, use standard)
            for a in [0.1,1.0,10.0]:
            ...
            for d in [8,None]:
                ds=str(d) if d else 'None'
                m = RandomForestRegressor(n_estimators=150,max_depth=d,random_state=42,n_jobs=-1)
                m.fit(Xf, y_fold, sample_weight=wf)
                loo_preds[f'rf_d{ds}_{fs_name}'][ti] = m.predict(Xtf)[0]
            for k in [3,5,10]:
                loo_preds[f'knn_k{k}_{fs_name}'][ti] = KNeighborsRegressor(
                    n_neighbors=k,weights='distance').fit(Xfs,y_fold).predict(Xtfs)[0]
            ...
            if fs_name == 'f30' and len(y_season) >= 5:
                ...
                if len(y_season) >= 6:
                    loo_preds[f'ps_knn_k5'][ti] = KNeighborsRegressor(
                        n_neighbors=5,weights='distance').fit(Xss,y_season).predict(Xtss)[0]
    ...
    # Model ranking by raw RMSE against GT (for evaluation, NOT for training)
    model_scores = [(n, np.sqrt(np.mean((M_loo[:,i] - test_gt)**2))) for i, n in enumerate(model_names)]
    model_scores.sort(key=lambda x: x[1])
    print(f"\n  {len(model_names)} models. Top-5:")
    ...
    # Also: blend with previous round predictions (multi-round consensus)
    this_mean = np.mean(M_loo, axis=1)
    all_round_preds.append(this_mean)
    
    for alpha in [0.3, 0.5, 0.7]:
        avg_past = np.mean(np.column_stack(all_round_preds[:-1]), axis=1)
        ensembles[f'blend_a{alpha:.1f}'] = alpha * this_mean + (1-alpha) * avg_past
    
    # Global average across all rounds
    ensembles['global_avg'] = np.mean(np.column_stack(all_round_preds), axis=1)
    ...
    # Update soft pseudo-labels: use best ensemble prediction (continuous)
    # Pick the ensemble with best assignment result
    best_ens = results[0][0]
    new_soft = ensembles[best_ens]
    change = np.sqrt(np.mean((new_soft - soft_pseudo)**2))
    print(f"  Soft label change RMSE: {change:.3f}")
    soft_pseudo = new_soft


# ════════════════════════════════════════════════════════════════
#  FINAL: Also try blending v17b's best with our predictions
# ════════════════════════════════════════════════════════════════
print(f"\n{'='*70}")
print("Final: Multi-round blend + grid search")
print("="*70)

# Grid search over all ensembles × powers
final_results = []
for ename, epred in ensembles.items():
    for power in [0.5, 0.75, 1.0, 1.05, 1.1, 1.15, 1.2, 1.25, 1.3, 1.5, 1.75, 2.0, 2.5, 3.0]:
        a = hungarian_assign(epred, test_seasons, avail_seeds, power)
        ex, sse, rmse = evaluate(a, test_gt)
        final_results.append((ename, power, ex, sse, rmse, a))

# Also blend round-0 and round-2 predictions (best rounds)
for alpha in np.arange(0.1, 0.95, 0.05):
    blended = alpha * all_round_preds[-1] + (1-alpha) * all_round_preds[0]
    for power in [1.0, 1.1, 1.25, 1.5]:
        a = hungarian_assign(blended, test_seasons, avail_seeds, power)
        ex, sse, rmse = evaluate(a, test_gt)
        final_results.append((f'cross_a{alpha:.2f}', power, ex, sse, rmse, a))

# Global average across all rounds
glob = np.mean(np.column_stack(all_round_preds), axis=1)
for power in [0.75, 1.0, 1.1, 1.25, 1.5, 2.0]:
    a = hungarian_assign(glob, test_seasons, avail_seeds, power)
    ex, sse, rmse = evaluate(a, test_gt)
    final_results.append(('global', power, ex, sse, rmse, a))

final_results.sort(key=lambda x: (-x[2], x[4]))
print(f"\n  Top-10 overall:")
for ename, pw, ex, sse, rmse, _ in final_results[:10]:
    print(f"    {ex}/91 RMSE/451={rmse:.4f} {ename}+p{pw}")

# Update global best
for ename, pw, ex, sse, rmse, a in final_results:
    if ex > best_exact or (ex == best_exact and rmse < best_rmse):
        best_assigned = a.copy()
        best_exact = ex
        best_rmse = rmse
...
saved = set(); sc_ = 0
for _,(cn, pw, ex, sse, rmse, assigned) in enumerate(final_results):
    if sc_ >= 5: break
    key = tuple(assigned)
